...
import {
    resetCurrentReaction } from "discourse/plugins/discourse-reactions/discourse/widgets/discourse-reactions-actions";
...
function customizeWidgetPostMenu(api) {
    ...
    api.addPostMenuButton('downvote', (post) => {
        ...
        return {
            action: ({ post, showFeedback }) => {
                ajax(
                    `/discourse-reactions/posts/${post.id}/custom-reactions/-1/toggle.json`,
                    { type: "PUT" }
                ).then((result) => {
    ...
    api.decorateWidget('post-menu:before-extra-controls', helper => {
        ...
        for (const reaction of model.reactions) {
            if (reaction.id === '+1') {
                num_upvotes = reaction.count;
            }

            if (reaction.id === '-1') {
                num_downvotes = reaction.count;
            }
        }
